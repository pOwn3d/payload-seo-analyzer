...
export function createKeywordResearchHandler(targetCollections: string[], globals: string[] = []): PayloadHandler {
  return async (req) => {
    try {
      ...
      const CACHE_KEY = reqLocale ? `keyword-research:${reqLocale}` : 'keyword-research'
      ...
      for (const { doc, sourceType, sourceSlug } of allFetched) {
        // Yield periodically — Lexical extraction over many docs is CPU-heavy.
        if (kProcessed > 0 && kProcessed % 50 === 0) await new Promise<void>((resolve) => setImmediate(resolve))
        kProcessed++
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const d = doc as any
      ...
      for (let i = 0; i < allDocs.length; i++) {
        if (i > 0 && i % 50 === 0) await new Promise<void>((resolve) => setImmediate(resolve))
        const doc = allDocs[i]
