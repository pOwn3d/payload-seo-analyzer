    ...
    try {
      ...
      while (hasMore) {
        ...
        for (const doc of result.docs) {
          ...
          // Cooperative yield after EVERY doc. analyzeSeo() is synchronous + CPU-heavy; running a
          // whole batch back-to-back blocks the event loop and makes the site unresponsive. Yielding
          // after each doc lets the single Node process serve site requests *between* analyses —
          // this is what makes the background build genuinely non-blocking.
