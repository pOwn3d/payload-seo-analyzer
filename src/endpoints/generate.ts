...
export function createGenerateHandler(
  ...
  return async (req) => {
    try {
      ...
      // Authorization (IDOR guard): only allow documents from the plugin's
      // configured target collections/globals. Without this, an authenticated
      // low-privilege user could read ANY collection/global via the generate fn.
      if (collectionSlug && !targetCollections.includes(collectionSlug)) {
        return Response.json({ error: 'Collection not allowed' }, { status: 403 })
      }
      if (globalSlug && !targetGlobals.includes(globalSlug)) {
