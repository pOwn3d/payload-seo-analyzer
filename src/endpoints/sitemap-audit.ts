  ...
  return async (req) => {
    try {
      ...
      for (const { doc, sourceSlug: collectionSlug } of allFetched) {
        ...
        // docs doesn't block other requests for the whole crawl.
      ...
      // 1b. Resolve collection-prefixed internal links (e.g. `/posts/<slug>` →
      // `posts/<slug>`) back to their canonical bare doc slug, so posts linked via
      // their public route are not mis-counted as broken links nor their targets as
      // orphans. Route prefixes = target collection slugs + configured knownRoutes.
      const allSlugs = new Set(allDocs.map((d) => d.slug))
      const routePrefixes = new Set(
        [...collections, ...knownRoutes].map((s) => s.toLowerCase().replace(/^\/+|\/+$/g, '')),
      )
      for (const links of outgoingMap.values()) {
        for (const link of links) {
          link.slug = resolveToDocSlug(link.slug, allSlugs, routePrefixes)
        }
      }
      ...
      // allSlugs + routePrefixes already computed above (step 1b).
