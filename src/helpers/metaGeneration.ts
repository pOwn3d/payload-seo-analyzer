/**
 * Shared meta generation helpers.
 * Used by both aiGenerate.ts and aiRewrite.ts endpoints.
 *
 * Heuristic meta title/description generation — no external AI API needed.
 */
...
export function truncateWords(text: string, maxLen: number, ellipsis = false): string {
...
export function pickPrefix(prefixes: string[], slug: string): string {
  let hash = 0
...
export function generateMetaTitle(title: string, focusKeyword: string | undefined, slug: string): string {
  if (!title || !title.trim()) {
