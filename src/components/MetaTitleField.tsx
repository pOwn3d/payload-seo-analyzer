...
function getCharColor(len: number): string {
  ...
  if (len >= TITLE_MIN && len <= TITLE_MAX) return C.green
  if (len > 0 && len < TITLE_MIN) return C.orange
  return C.red
}

function getProgressPercent(len: number): number {
  if (len === 0) return 0
  return Math.min((len / TITLE_MAX) * 100, 100)
}

function getProgressColor(len: number): string {
  if (len === 0) return 'var(--theme-elevation-200, #e5e7eb)'
  if (len >= TITLE_MIN && len <= TITLE_MAX) return C.green
  if (len < TITLE_MIN) return C.orange
  return C.red
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {charCount} / {TITLE_MAX} {t.metaTitle.characters}
        </span>
      </div>
      ...
      >
        <span>
          {t.metaTitle.optimal} {TITLE_MIN}-{TITLE_MAX} {t.metaTitle.characters}
        </span>
        <span
          style={{
            fontWeight: 700,
        ...
        >
          {charCount < TITLE_MIN
            ? `${TITLE_MIN - charCount} ${t.metaTitle.charactersMissing}`
            : charCount > TITLE_MAX
              ? `${charCount - TITLE_MAX} ${t.metaTitle.charactersTooMany}`
              : t.metaTitle.idealLength}
        </span>
      </div>
 ...
 */
export const MetaTitleField = withSeoErrorBoundary(MetaTitleFieldInner, { viewName: 'MetaTitleField' })
