...
export interface AnalysisContext {
  ...
  fullText: string
  /**
   * Same text as fullText but with sentence boundaries at block ends (paragraphs,
   * list items, headings). Used for readability metrics (Flesch + sentence
   * segmentation) so scannable list-heavy content is not merged into giant
   * pseudo-sentences. Use fullText for word count / keyword density.
   */
