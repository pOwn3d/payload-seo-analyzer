...
export function extractTextFromLexical(
  ...
  sentenceBoundaries: boolean = false,
): string {
  return _extractText(node, 0, maxDepth, sentenceBoundaries)
}
...
function _extractText(
  ...
  if (Array.isArray(n.children)) {
    const joined = n.children
      .map((child: unknown) => _extractText(child, depth + 1, maxDepth, sentenceBoundaries))
      .join(' ')
    // When sentenceBoundaries is enabled (readability analysis only), block-level
    // nodes terminate a sentence. Without this, list items and paragraphs — which
    // frequently lack trailing punctuation — get concatenated into one giant
    // pseudo-sentence, which collapses the sentence count and wrongly tanks the
    // Flesch score on scannable, list-heavy content. Off by default so word count,
    // keyword density, and heading/title extraction keep their clean output.
    if (sentenceBoundaries) {
      const t = n.type
      if (t === 'paragraph' || t === 'heading' || t === 'listitem' || t === 'quote') {
  ...
  if (n.root && typeof n.root === 'object') {
    return _extractText(n.root, depth + 1, maxDepth, sentenceBoundaries)
  }
