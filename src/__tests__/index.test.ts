...
describe('analyzeSeo', () => {
  ...
  it('exposes a separate AI-readiness sub-score (SEO 2026)', () => {
    const result = analyzeSeo(makeFullInput())
    expect(result.aiReadiness).toBeDefined()
    expect(result.aiReadiness!.score).toBeGreaterThanOrEqual(0)
    expect(result.aiReadiness!.score).toBeLessThanOrEqual(100)
    expect(result.aiReadiness!.checkCount).toBeGreaterThan(0)
    expect(['poor', 'ok', 'good', 'excellent']).toContain(result.aiReadiness!.level)
  })
