...
export function checkFreshness(input: SeoInput, ctx: AnalysisContext): SeoCheck[] {
  ...
  // 5. Fake "refresh" — the date shown to readers is meaningfully NEWER than the
  // actual last modification (updatedAt). Real freshness = a genuine content update,
  // not a bumped date. Conservative threshold (60 days) + informational (weight 0):
  // Google detects the real date anyway, so this is guidance, not a penalty.
  if (input.displayedDate && input.updatedAt) {
    ...
    if (
      displayedDays !== Infinity &&
      updatedDays !== Infinity &&
      updatedDays - displayedDays > 60
    ) {
      checks.push({
