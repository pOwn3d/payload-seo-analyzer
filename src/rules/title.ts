...
// Google truncates SERP titles by PIXEL width (~580-600px), not character count.
const TITLE_PIXEL_MAX = 600
const NARROW_CHARS = new Set("iIl.,:;|!'`jft()[]{}/\\".split(''))
const WIDE_CHARS = new Set('mwMW@%'.split(''))

/**
 * Rough pixel-width estimate of a title at the SERP font (~20px sans-serif).
 * Informational only — a proxy so editors can spot likely truncation.
 */
function estimateTitlePixelWidth(title: string): number {
  ...
  for (const ch of title) {
    ...
    else px += 9 // typical lowercase / accented / other glyph
  }
...
export function checkTitle(input: SeoInput, ctx: AnalysisContext): SeoCheck[] {
  ...
  // 2b. Title pixel width — informational (SEO 2026: SERP truncates by pixels, not chars)
  ...
  // Headline analyzer — Number in title (listicle-friendly, may help CTR — bonus only)
